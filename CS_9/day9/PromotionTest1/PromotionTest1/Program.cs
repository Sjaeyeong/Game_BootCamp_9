...
namespace PromotionTest1
{
    internal class Program
    {
        ...
        class Player
        {
            ...
            public void ChooseHand()            // 손 입력 받기 + 함수결과 : _hand에 입력받은 손 저장
            {
                    ...
                    switch (menuSelect)
                    {
                        case 0:
                            while (_round <= 5 && _player._money > 0)
                            {
                                ...
                                _gameDisplay.ShowRoundStatus(_round, _player._money, _stackGold);

                                _player.Betting();
                                _player.ChooseHand();
                                _computer.ChooseHand();

                                GameResult result = Judge(_player._hand, _computer._hand);

                                _gameDisplay.ShowBattle(_player._hand, _computer._hand);
                                _gameDisplay.ShowRoundResult(result);

                                CalculateResult(result);

                                _gameDisplay.WaitForEnter();

                                _round++;
                            }
                            _gameDisplay.ShowFinalResult(_winCount, _loseCount, _drawCount, _player._money);
                            break;
                        case 1:
                            _gameDisplay.PrintLine();
                            _gameDisplay.ShowGameRule();
                            _gameDisplay.PrintLine();
